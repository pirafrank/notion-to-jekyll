...
const loadCache = (cachePath) => {
  try {
    ...
    cache = require(config.jekyllRoot + "/.notion-to-jekyll.json");
    console.log(
      `Loaded cache from ${config.jekyllRoot}/.notion-to-jekyll.json, version ${cache.version}`
    );
  } catch (error) {
...
const putAssetOffline = async (url, iteration, assetId) => {
  ...
  if (!cacheEntry) {
    ...
    // first download...
    const writtenFile = await downloadBinaryStream(url, iteration, filename);
    ipc.push(ipc.data.changed, writtenFile);
    console.log(`Downloaded asset to: ${writtenFile}`)
    // ... then write to cache
    cache.data[assetId] = publishedUrl;
    const cachePath = `${config.jekyllRoot}/.notion-to-jekyll.json`;
