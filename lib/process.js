  ...
  try {
    if (!config.dryRun) {
      ...
      if (config.publishToPosts) {
        ...
        await updatePublishedByProperty(page.id, config.notionToJekyllUser);
      }
    } else {
      console.log(`** Dry-run mode ** : would have updated page properties and written file: ${filename}`);
    }
  } catch (err) {
