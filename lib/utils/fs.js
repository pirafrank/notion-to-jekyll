...
const createDirectory = (dirPath) => {
  ...
  directories.forEach((dir) => {
    ...
    if (!fs.existsSync(currentPath)) {
      if (dryRun) {
        dry(`would have created directory: ${currentPath}`);
        return;
      }
...
const copyFile = (source, target, honorDryRun) => {
  if (getConfig().dryRun && honorDryRun) {
    dry(`would have copied file "${source}" to "${target}"`);
    return;
  }
  fs.copyFileSync(source, target);
  ipc.push(ipc.changed, target);
...
const deleteFileIfExistsSync = (filepath) => {
  if (fs.existsSync(filepath)) {
    ...
    info(`Skipping file deletion, file does not exist: ${filepath}`);
  }
};
