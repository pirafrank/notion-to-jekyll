...
const processPage = async (page, config) => {
  ...
  processResult.targetAssetPath = path.join(config.assetsPath, targetAssetDir.toString());
  ...
  try {
    ...
    error(`Error converting properties to frontmatter for page '${processResult.pageName}'.`, err.message);
  ...
  const filename = calculateMdFilename(date, processResult.pageSlug);
  const filepath = path.join(outputPath, filename);

  // check if file already exists in target dir
  const match = checkForFileInFolder(outputPath, processResult.pageSlug);
...
Deleting matching page in dir before writing new file...`
    ...
    try {
      if (!config.dryRun) {
        unlinkSync(path.join(outputPath,match));
        processResult.action = processResult.action + "DeleteOk";
      } else {
        info(`** Dry-run mode ** : would have deleted file: ${match}`);
      }
    } catch (err) {
      processResult.action = processResult.action + "OnErrorDeleteFile";
      error(`Error deleting file ${match}: ${err.message}`);
      return processResult;
    }
  }
  ...
  try {
    if (!config.dryRun) {
      ...
      info(
        `Updating page properties for page: ${processResult.pageName}`
      );
      //if (config.publishToPosts) {
        // update the page properties to mark it as published
      ...
      // } else {
      //   // update the page property to mark it as processed by the script
      //   await updatePublishedByAndStatusProperties(
      //     page.id,
      //     config.notionToJekyllUser,
      //     config.notionDoneStatus
      ...
      info(`** Dry-run mode ** : would have updated page properties and written file: ${filepath}`);
    }
  } catch (err) {
    processResult.action = processResult.action + "WriteOnError";
    error(`Error writing file ${filename}: ${err.message}`);
  }
  return processResult;
};
...
const parseResults = async (results, config) => {
  ...
  for (let i = 0; i < results.length; i++) {
    ...
    try {
      ...
      error(`Error while processing page '${pageName}'. Skipping page.`, err.message);
    }
  }
};
