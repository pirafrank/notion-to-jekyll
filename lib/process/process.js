...
const calculateMdFilename = (date, pageSlug) => {
  ...
  info(`Calculated filename "${filename}" for page slug "${pageSlug}"`);
  return filename;
};
...
const processPage = async (page, config) => {
  ...
  processResult.pageDate = config.date;
  ...
  const filename = calculateMdFilename(processResult.pageDate, processResult.pageSlug);
