...
const calculateJekyllAssetUrl = (targetAssetDir, filename) => {
  const config = getConfig();
  return `{{ site.baseurl }}/${config.assetsDir}/${targetAssetDir}/${filename}`;
};

const calculateJekyllAssetPath = (targetAssetDir, filename) => {
  const config = getConfig();
  return `/${config.assetsDir}/${targetAssetDir}/${filename}`;
};

const parseAssetPathFromPublishedUrl = (publishedUrl) => {
  const parts = publishedUrl.split("/");
  return {
    targetAssetDir: parts.at(-2),
    filename: parts.at(-1),
  };
};
...
const putAssetOffline = async (url, iteration, assetId) => {
  ...
  if (!cacheEntry) {
    publishedUrl = calculateJekyllAssetUrl(
    ...
    ipc.push(ipc.changed, writtenFile);
    ...
    const { targetAssetDir, filename: cachedFilename } =
      parseAssetPathFromPublishedUrl(cache[assetId]);
    publishedUrl = calculateJekyllAssetUrl(targetAssetDir, cachedFilename);
  }
  return {
    filename: filename,
...
module.exports = {
  putAssetOffline,
  writeAccordionFile,
  calculateJekyllAssetUrl,
  calculateJekyllAssetPath,
};
