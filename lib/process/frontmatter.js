...
const { putAssetOffline, calculateJekyllAssetPath } = require('./fetch');
...
// seoimage: "/static/postimages/300x/some-pic-seo.jpg"
// series: "My Series"   # optional, only when Series and Part are both set
// part: 1               # optional, only when Series and Part are both set
// ---
//
...
const extractPropertiesFromPage = async (iteration, page) => {
  ...
  // optional series metadata: include only when both Series and Part are present
  const series = properties?.Series?.rich_text?.[0]?.plain_text;
  const part = properties?.Part?.number;
  if (series && part != null) {
  ...
  try {
    ...
    if (seoimageUrl) {
      ...
      frontmatter.seoimage = calculateJekyllAssetPath(
...
const convertPropertiesToJekyllFrontmatter = async (metadata, page) => {
  info("Converting Notion page properties to Jekyll frontmatter");
  try {
    const frontmatter = await extractPropertiesFromPage(metadata, page);
    validateFrontmatter(frontmatter);
    return getFrontmatterAsString(frontmatter);
  } catch (err) {
    error(`Error converting properties to frontmatter for page `
      +`'${metadata.pageName}'.`, err.message);
    throw err;
  }
};
